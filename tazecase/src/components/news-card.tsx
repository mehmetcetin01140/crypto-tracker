...
export default function NewsCard({ isDetailPageRendering }: Props) {
  const { data } = useQuery(["fetchnewsdetail", ""], () =>
    fetchNews("")
  );
  const { id } = useParams();
  
  const showNewsOtherThanDisplayedNewsOnDetailPage: NewsTypes[] |undefined =
    isDetailPageRendering
      ? data?.filter((news: NewsTypes) => news.id !== Number(id))
      : data;
    ...
    <Row xs={1} md={1} lg={isDetailPageRendering ? 1 : 2} className="g-4">
      {showNewsOtherThanDisplayedNewsOnDetailPage?.map((news: NewsTypes) => (
        ...
        >
          <Col>
            <Card
              style={{ height: isDetailPageRendering ? "auto" : 490 }}
              className={!isDetailPageRendering ? "news-card" : ""}
            >
              <Card.Img
                variant="top"
                src={news.image}
